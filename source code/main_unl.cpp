...
//#define Video_Buffer_Len 25*2   // based on the frame rate it should be 1,2,3,4,5 seconds and related value
//#define Shrink_Speed 3     // set the value of HIGH in velocity+ update method
//#define ViewScaler 1.2     //123 number


//int Video_Divide = 10;      // (tile number)indicate how we devide the video, the total number should be Video_Divide*Video_Divide
int Video_Buffer_Len = 25 * 2;  // based on the frame rate it should be 1,2,3,4,5 seconds and related value
int Shrink_Speed = 3;     // set the value of HIGH in velocity+ update method
int ViewScaler = 1;    //123 number

#define ShowFeature 1
#define MASK 1
int BlockStatus[Video_Divide*Video_Divide];
int BlockTimeSlot[Video_Divide*Video_Divide];
void InitBlockTimeSlot()
{
	int i = 0;
...
void UpdateBlockByOther(int * blocklist, IplImage *frame,
	...
	int C_XH, int C_XL, int C_YH, int C_YL
)
{
	int i = 0;
	int L = frame->width;
	...
	for (i = 0; i < Video_Divide*Video_Divide; i++)
	{
		...
		if (((XH <= A_XH) && (XH >= A_XL) && (YH <= A_YH) && (YH >= A_YL)) ||
			...
			((XH <= C_XH) && (XH >= C_XL) && (YH <= C_YH) && (YH >= C_YL)) ||
			((XL <= C_XH) && (XL >= C_XL) && (YH <= C_YH) && (YH >= C_YL)) ||
			((XH <= C_XH) && (XH >= C_XL) && (YL <= C_YH) && (YL >= C_YL)) ||
			((XL <= C_XH) && (XL >= C_XL) && (YL <= C_YH) && (YL >= C_YL))
			)
		{
			blocklist[i] = 1;
			//BlockStatus[i] = Shrink_Speed;
			//if (BlockStatus[i] < 7) BlockTimeSlot[i]++;
			BlockStatus[i] = BlockTimeSlot[i];
		}
	}
}
...
void UpdateSmallBlocksSet(int * blocklist, IplImage *frame, int x, int y)
{
	...
	BlockStatus[BlockIndex] = BlockTimeSlot[BlockIndex];
}
...
int Notmain(string videoname, string testvideoname, int KTime)
{
	//========================add timer=====================================
	...
	//string videoname = "stonemilker";
	NameStr = NameStr + videoname;
	videoname = "A"+ videoname + ".mp4";
	VideoCapture cap(videoname);     // the orignal is   ElephantsOldB  ||  ElephantsC || Wild_Horses ||5cutecat || cow || pretty
										//Security   giraf   sharks  Clash_of_Clans  stonemilker   Nikefootball   AIRbus
	...
	testvideoname = testvideoname + ".mp4";
	VideoCapture capb(testvideoname);            //cowrecord   compareD  catrecord
	if (!capb.isOpened())
	{
		return -1;
	}
	/*
	|  video 1.ElephantsC 2.Wild_Horses 3.5cutecat 4.cow 5.pretty 6.Security 7.giraf 8.sharks 9.Clash_of_Clans 10.stonemilker 11.Nikefootball 12.AIRbus
	| 1. Surrounded by Wild Elephants in 4k 360
	| 2. 360 VR Video of Namib Desert Wild Horses at Klein-Aus Vista
	| 3. 5 Cute Cats VR (360° video)
	| 4. 360° Getting Licked by a Cow in Ireland 4k
	| 5. [360 VR] 밤비노 (Bambino) '오빠오빠' Eyes mode
	| 6. Security Cam (360 VR video!)
	| 7. 360 VR Video of Giraffe Manor, The Safari Collection - Kenya
	| 8. 360°, Diving with sharks. 8K Underwater video
	| 9. Clash of Clans 360- Experience a Virtual Reality Raid
	| 10. björk- stonemilker (360 degree virtual reality)
	| 11. Nike Hypervenom II The Neymar Jr Effect, A Virtual Reality Experience
	| 12. 360° cockpit view - SWISS Airbus A320 - Geneva – Zurich
	*/
	Mat UsrFrame_RGB;
	Mat UsrFrame_GRY;
	Mat BgFrame_GRY;
	...
	cv::Point minLoc, maxLoc, matchLoc, matchLoc_new, matchLoc_old;
	...
	Filename_TXT = NameStr + Filename_TXT+ testvideoname + NameB.str() + "S_" + NameA.str() + "T_" + NameC.str() + "V_"+NameD.str()+"SF" + ".txt";
	...
	Filename_AVI = NameStr + Filename_AVI + testvideoname + NameB.str() + "S_" + NameA.str() + "T_" + NameC.str() + "V_" + NameD.str() + "SF" + ".avi";
	...
	while (true)
	{
		...
		** velocity based prediction when one buffer lenth
		...
		{
			VBP_VX_Start = matchLoc_old.x;
			VBP_VY_Start = matchLoc_old.y;
		}
		if (Usr_V_Count >= Video_Buffer_Len)
		{
			...
			VBP_XH = matchLoc_old.x + windw;
			VBP_XL = matchLoc_old.x;
			VBP_YH = matchLoc_old.y + windh;
			//VBP_YH = matchLoc_old.y + image2.rows;
			VBP_YL = matchLoc_old.y;
			//==============update the Basic + enhancedA prediction================
			//ENHAN_A_XH = matchLoc.x + image2.cols;
			ENHAN_A_XH = matchLoc_old.x + windw;
			ENHAN_A_XL = matchLoc_old.x;
			//ENHAN_A_YH = matchLoc_old.y + image2.rows;
			ENHAN_A_YH = matchLoc_old.y + windh;
			ENHAN_A_YL = matchLoc_old.y;
			//=============update the shrink prediction=============================
			MaintainBlocks(Block_List);

			matchLoc_old = matchLoc;
		}
		...
		** Update and show the velocity based prediction result
		...
		3);*/
#if ShowFeature
		//show the usr view  Scalar(B, G, R)                                                                                                                 
		...
		** Update and show the shrinking based prediction result
		...
		UpdateBlockByOther(Block_List, frame1,
			...
			windx + windw, windx, windy + windh, windy ,
			VBP_XH, VBP_XL, VBP_YH, VBP_YL,
		...
		Usr_V_XH = Usr_V_XH - int(30 * ViewScaler);
		Usr_V_YH = Usr_V_YH - int(30 * ViewScaler);
		Usr_V_XL = Usr_V_XL + int(30 * ViewScaler);
		Usr_V_YL = Usr_V_YL + int(30 * ViewScaler);
...
// the orignal is   ElephantsOldB  ||  ElephantsC || Wild_Horses ||5cutecat || cow || pretty
//Security   giraf   sharks  Clash_of_Clans  stonemilker   Nikefootball   AIRbus
int main(int argc, char *argv[])
{
	string Name= "Cow";
	string NameTest = "Record_A_"+ Name;
	int i, j,k;
	int l, m, n;
	for(i=4;i<5;i++)   //0->10
		for (j = 0; j < 4; j++)
		{
			switch (i)
			{
			case 0:Name = "Cow"; k = 404; break;
			case 1:Name = "Wild_Horses"; k = 404; break;
			case 2:Name = "pretty"; k = 804; break;
			case 3:Name = "Security"; k = 404; break;
			case 4:Name = "giraf"; k = 360; break;
			case 5:Name = "sharks"; k = 804; break;
			case 6:Name = "Clash_of_Clans"; k = 404; break;
			case 7:Name = "stonemilker"; k = 804; break;
			case 8:Name = "Nikefootball"; k = 404; break;
			case 9:Name = "AIRbus"; k = 804; break;
			case 10:Name = "5cutecat"; k = 404; break;
			default:
				break;
			}
			switch (j)
			{
			case 0:NameTest = "Record_A_" + Name; break;
			case 1:NameTest = "Record_B_" + Name; break;
			case 2:NameTest = "Record_C_" + Name; break;
			case 3:NameTest = "Record_D_" + Name; break;
			default:
				break;
			}
			/*
			Video_Buffer_Len = 25 * 1;  
			Shrink_Speed = 3;     
			ViewScaler = 1;
			Notmain(Name, NameTest,k);
			*/
			for (l = 1; l < 5; l++)   //buffer len 1 2 3 4
			{
				Video_Buffer_Len = 25 * l;
				for (n = 1; n < 4; n++)   //scaler  1 2 3 
				{
					ViewScaler = n;
					Notmain(Name, NameTest, k);
				}
	...
	//Notmain("Wild_Horses", "Record_A_Wild_Horses");
	return 0;
}